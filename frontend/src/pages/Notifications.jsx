...
import { Bell, Check, CheckCheck, Trash2, Megaphone, AlertCircle, Info, CheckSquare, AlertOctagon, Radio } from 'lucide-react';
...
export const Notifications = ({ currentUser }) => {
  ...
  const [type, setType] = useState('EMERGENCY_ALERT');
    ...
    <div className="notifications-page">
      <div className="page-header">
        <div className="page-title-group">
          ...
          <p>Real-time emergency bulletins, SOS alerts, voice alerts, task assignments, and announcements.</p>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {notifications.length === 0 ? (
          ...
          notifications.map(n => {
            const isCritical = n.type === 'EMERGENCY_ALERT' || n.type === 'SOS_ALERT';
            const isVoice = n.type === 'VOICE_ALERT';
            ...
            return (
              <div
                ...
                style={{
                  ...
                  gap: '1rem',
                  borderLeft: isCritical ? '4px solid #ef4444' : isVoice ? '4px solid #f97316' : n.is_read ? '1px solid var(--border-glass)' : '4px solid var(--accent-primary)',
                  opacity: n.is_read ? 0.75 : 1
                }}
              >
                <div className={`stat-icon-wrapper ${isCritical ? 'stat-icon-rose' : 'stat-icon-sky'}`} style={{ width: '42px', height: '42px', flexShrink: 0 }}>
                  {n.type === 'SOS_ALERT' ? <AlertOctagon size={20} color="#ef4444" /> :
                   n.type === 'EMERGENCY_ALERT' ? <AlertCircle size={20} color="#ef4444" /> :
                   n.type === 'VOICE_ALERT' ? <Radio size={20} color="#f97316" /> :
                   <Bell size={20} />}
                </div>

                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                    <strong style={{ fontSize: '1rem', color: isCritical ? '#fca5a5' : 'var(--text-primary)' }}>{n.title}</strong>
